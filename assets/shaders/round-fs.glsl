...
uniform vec3 uRoundColourA;
uniform vec3 uRoundColourB;
...
void main()
{
    ...
    vec3 colourA = uRoundColourA;
    vec3 colourB = uRoundColourB;
    // vec3 colourA = vec3(0.0);
    // vec3 colourB = vec3(1.0, 0.0, 0.0);
    ...
    float gradient = Grad3(-0.1,1.0,0.0,0.42,uv0.y - .03);
    float darkGradient = f * gradient * 0.1;

    // float fGrad = 1.-gradient;
    float fGrad = gradient;

    vec3 black = mix(colourA,colourB,f * smoothstep(0.0,1.50,(1.-fGrad)));
